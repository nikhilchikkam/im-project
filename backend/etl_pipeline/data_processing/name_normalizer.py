...
def update_normalized_names_batched():
    with engine.begin() as conn:
        # Only get products where normalized_name is NULL and nutrient_available is TRUE
        ...
        logging.info(f'Fetched {len(gtins)} products with nutrient_available = TRUE and normalized_name IS NULL.')
        
        if not gtins:
            logging.info('No products found that need normalized_name updates.')
            return
