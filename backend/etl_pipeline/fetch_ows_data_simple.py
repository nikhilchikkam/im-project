...
- Fetches 1000 products per API call
...
def fetch_and_store_products(continue_from_last=False):
    """Fetch 1000 products from OneWorldSync and store in Spaces"""
    
    print("🚀 Starting OneWorldSync data fetch (1000 products per batch)...")
    ...
    try:
        ...
        # Build criteria - always fetch 1000 products
        criteria = {
            "fields": {
        ...
        if search_after:
            ...
            print(f"🔄 Fetching next 1000 products from OneWorldSync...")
        else:
            print(f"🔄 Fetching 1000 products from OneWorldSync...")
        ...
        # Store products in a single file (1000 products per file)
        print("🔄 Storing products in DigitalOcean Spaces...")
        ...
        batch_key = f"batches/{batch_id}/products.json"
        spaces_client.put_object(
            Bucket=bucket,
            Key=batch_key,
        ...
        print(f"✅ Batch stored: {batch_key}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fetch OneWorldSync data to DigitalOcean Spaces (Simple)')
    ...
    success = fetch_and_store_products(args.continue_from_last)
